...
app.use(express.json());//用来解析request.body的！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
...
app.post("/admin/login",function(request,response){
  ...
  let login_name_request = request.body.login_name;
  let password_request = request.body.password;
  User.findOne({login_name:login_name_request},(err,user)=>{
    if(err||!user){
      response.status(400).send(`${login_name_request} isn't a valid account name`);
      console.log("not found user");
      return;
    }
    else{
      if(user.password !== password_request){
        response.status(400).send(`Wrong password, please try again.`);
        ...
        let {_id, first_name, last_name, location, description, occupation,login_name} = user;
        let newUser = {_id, first_name, last_name, location, description, occupation,login_name};
        response.status(200).send(newUser);
        console.log(newUser);
        return;
      }
...
app.post("/admin/logout",function(request,response){
  request.session.destroy(function(err){
    if(err){
      response.status(400).send("Unable to destroy session, can't log out.");
    }
    else{
